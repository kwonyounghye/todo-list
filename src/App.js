...
const List = () => {
  ...
  const [edit, setEdit] = useState(null); // 편집중?
...
const onEdit = useCallback((id, value) => {
  const update = prompt('Edit To-Do');
  if(update !== null && update.trim() !== '') {
    setAdd(items => items.map((item, index) => index === id ? update : item));
  ...
  return (
    <div className='container'>
     ...
     <button type='submit' onClick={onSubmit}>ADD</button>
      ...
      <div className='list'>
      <TodoList add={add} onRemove={onRemove} onEdit={onEdit} />
      </div>
    </div>
  )
}
